...
class BlackScholes( object ):
    ...
    def _N_deriv( self, d ):
        return derivative( self._N, d, dx=1e-6 )
    ...
    def gamma( self ):
        return self._N_deriv( self.d1 ) / ( self.S_0 * self.volatility * sqrt( self.T ) )

    def vega( self ):
        return self.S_0 * sqrt( self.T ) * self._N_deriv( self.d1 )

    def theta( self ):
        return -1 * self.volatility * self.S_0 * self._N_deriv( self.d1 ) / ( 2 * sqrt( self.T ) ) - self.r * self.K * exp( -1 * self.r * self.T ) * self._N( self.d2 )
